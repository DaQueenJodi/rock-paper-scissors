        ...
        for (offsets) |dy| {
            for (offsets) |dx| {
                ...
                if (flavor.beats(curr_flavor)) {
                    count += 1;
                    if (count > treshold.?) return pixel;
                }
            }
        }
        return null;
...
fn parseSize(str: []const u8) !struct { width: usize, height: usize } {
    ...
    return .{
        .width = std.fmt.parseInt(usize, width_str, 10) catch return error.FailedToParse,
        .height = std.fmt.parseInt(usize, height_str, 10) catch return error.FailedToParse
    };
}
    ...
    while (args.next()) |arg| {
        switch (state) {
            .nothing => {
                ...
                if (eql(arg, "--image")         or eql(arg, "-i")) state = .image
                else if (eql(arg, "--width")    or eql(arg, "-w")) state = .width
                else if (eql(arg, "--height")   or eql(arg, "-h")) state = .height
                else if (eql(arg, "--treshold") or eql(arg, "-t")) state = .treshold
                else {
...
pub fn main() !void {
    ...
    const image = blk: {
        if (config.image) |path| {
            ...
            break :blk .{
                .pixels = c.LoadImageColors(image)[0..width*height],
                .width = width,
                .height = height
            };
        } else {
